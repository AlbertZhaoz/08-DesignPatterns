...
namespace _1_单例模式
{
    public class LockLazyMan
    {
        private LockLazyMan(){}

        // volatile 容易改变的，不稳定的，不确定的
        // 避免指针重排
        private static volatile LockLazyMan _lockLazyMan;

        private static object o = new object();

        public static LockLazyMan GetLockLazyMan()
        {
            // 为了尽可能避免锁竞争，造成的资源浪费
            if (_lockLazyMan == null)
            {
                ...
                lock (o)
                {
                    if (_lockLazyMan == null)
                    {
                        ...
                        _lockLazyMan = new LockLazyMan();
                        Console.WriteLine($"LockLazyMan is created by {Thread.CurrentThread.ManagedThreadId}");
                    }
                }
            }
